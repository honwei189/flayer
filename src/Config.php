...
class Config
{
    ...
    public static function load($config_file_name = null)
    {
        if (count(self::$config) == 0) {
            ...
            if (is_value(self::$path)) {
                $path = self::$path;
            } else {
                if (php_sapi_name() == "cli") {
                    if ($_SERVER['cwd'] ?? false) {
                        $path = $_SERVER['cwd'] . DIRECTORY_SEPARATOR;
                    } else {
            ...
            if (is_dir($path . "config")) {
                if (is_value($config_file_name) && file_exists($path . "config" . DIRECTORY_SEPARATOR . $config_file_name . ".php")) {
